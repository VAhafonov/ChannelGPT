...
class GPT():
    def __init__(self, channel_language: str, channel_name: str, channel_theme: str, temperature: bool, debug: bool = False):
    ...
    def CreatePost(self):
        ...
        with open("messages.txt", "r") as file:
            ...
            if content!="" and content!="\n":
                bot_messages = content.split("\end_of_post")
                if len(bot_messages)>5:
                    bot_messages = bot_messages[-5:]
                for message in bot_messages:
                    self.messages.append({"role": "system", "content": message})
                    self.messages.append({"role": "user", "content": "create"})
        
        r = self.call(temperature=self.temperature, messages=self.messages)
        if self.debug:
            print(r)
        return r
